...
#interrogation experiment 2008-2009
#look at top comment for explanation of data set creation
interrog_experiment1 = exonerations%>%
  ...
  mutate(time_served = exonerated-convicted,
         control = date_recordinginterrog>end_date+modifier,
         treatment = (date_recordinginterrog<=end_date&date_recordinginterrog>=beg_date),
...
#interrogation experiment 2010-2011
#look at top comment for explanation of data set creation
interrog_experiment2 = exonerations%>%
  filter((exonerated>=end_date)|(exonerated<=beg_date)&((exonerated<=end_date+modifier)&(exonerated>=beg_date-modifier)))%>%
  mutate(date_witnessreform = ifelse(date_witnessreform=="None",2025,date_witnessreform),
         date_recordinginterrog = ifelse(date_recordinginterrog=="None",2025,date_recordinginterrog),)%>%
  mutate(time_served = exonerated-convicted,
...
#interrogation experiment 2012-2013
#look at top comment for explanation of data set creation
interrog_experiment3 = exonerations%>%
  filter((exonerated>=end_date)|(exonerated<=beg_date)&((exonerated<=end_date+modifier)&(exonerated>=beg_date-modifier)))%>%
  mutate(date_witnessreform = ifelse(date_witnessreform=="None",2025,date_witnessreform),
         date_recordinginterrog = ifelse(date_recordinginterrog=="None",2025,date_recordinginterrog),)%>%
  mutate(time_served = exonerated-convicted,
...
#interrogation experiment combined
#This aggragate the previous three time frames datasets into a single dataset
interrog_experiment_comb = interrog_experiment1%>%
  bind_rows(interrog_experiment2)%>%
  bind_rows(interrog_experiment3)

#The following is the contol and treatment sizes for each experiment
#This is reflected in table 1 of the paper
sum(interrog_experiment1$control, na.rm = TRUE)
sum(interrog_experiment1$treatment, na.rm = TRUE)
sum(interrog_experiment2$control, na.rm = TRUE)
sum(interrog_experiment2$treatment, na.rm = TRUE)
sum(interrog_experiment3$control, na.rm = TRUE)
sum(interrog_experiment3$treatment, na.rm = TRUE)
...
fit_ds1ctrl = lm(dna_substantial ~ after_target + treatment + treatment*after_target + statepop + race, data = interrog_experiment1)
fit_fc1ctrl = lm(fc ~ after_target + treatment + treatment*after_target + statepop + race, data = interrog_experiment1)
fit_mw1ctrl = lm(mwid ~ after_target + treatment + treatment*after_target + statepop + race, data = interrog_experiment1)
fit_ds2ctrl = lm(dna_substantial ~ after_target + treatment + treatment*after_target + statepop + race, data = interrog_experiment2)
fit_fc2ctrl = lm(fc ~ after_target + treatment + treatment*after_target + statepop + race, data = interrog_experiment2)
fit_mw2ctrl = lm(mwid ~ after_target + treatment + treatment*after_target + statepop + race, data = interrog_experiment2)
fit_ds3ctrl = lm(dna_substantial ~ after_target + treatment + treatment*after_target + statepop + race, data = interrog_experiment3)
fit_fc3ctrl = lm(fc ~ after_target + treatment + treatment*after_target + statepop + race, data = interrog_experiment3)
fit_mw3ctrl = lm(mwid ~ after_target + treatment + treatment*after_target + statepop + race, data = interrog_experiment3)
fit_dsCctrl = lm(dna_substantial ~ after_target + treatment + treatment*after_target + statepop + race, data = interrog_experiment_comb)
fit_fcCctrl = lm(fc ~ after_target + treatment + treatment*after_target + statepop + race, data = interrog_experiment_comb)
fit_mwCctrl = lm(mwid ~ after_target + treatment + treatment*after_target + statepop + race, data = interrog_experiment_comb)



#This is the code for table 11: looking at the effect of interrogation reocrding on
#DNA playing a substantial role in the exoneration
stargazer(fit_ds1ctrl,fit_ds2ctrl,fit_ds3ctrl,
          title = "Recording Interrogations",
          dep.var.labels = c("DNA Substantial"),
          column.labels = c("2008-2009","2010-2011","2012-2013"),
...
#This is the code for table 12: looking at the effect of interrogation recording on
#false confession
...
#This is the code for table 13: looking at the effect of interrogation recording on
#mistaken witness ID
...
#This is the code for table 4: looking at the effect of interrogation recording on
